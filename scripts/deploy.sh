...
for i in $(seq 1 "$MAX_RETRIES"); do
    ...
    # docker exec chạy curl bên trong container — không cần port expose ra host
    # QUAN TRỌNG: không dùng '|| echo 000' vì curl đã output 000 qua -w khi fail
    # nếu dùng || echo 000 sẽ thành 000000 và không bao giờ match "200"
